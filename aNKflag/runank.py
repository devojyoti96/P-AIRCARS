	...
	def flag_params(self,flagmode,chanfrac=0.0,timefrac=0.0):
		...
		if flagmode==0:
			return [['vis_ind','mean','median','re',1.8,0.3,1,'',0,0,0.0,0.0],['vis_ind','mean','median','im',1.8,0.3,1,'',0,0,0.0,0.0],\
					['vis_ind','mean','median','re',1.7,0.3,1,'',0,0,0.0,0.0],['vis_ind','mean','median','im',1.7,0.3,1,'',0,0,0.0,0.0]]
		elif flagmode==1:
			return [['chan_ind','mean','median','re',1.8,0.3,1,'',0,0,0.0,0.0],['chan_ind','mean','median','im',1.8,0.3,1,'',0,0,0.0,0.0],\
					...
					['chan_ind','mean','median','re',1.7,0.3,1,'',0,0,0.0,0.0],['chan_ind','mean','median','im',1.7,0.3,1,'',0,0,0.0,0.0],\
					['vis_ind','mean','median','re',1.7,0.3,1,'',0,0,0.0,0.0],['vis_ind','mean','median','im',1.7,0.3,1,'',0,0,0.0,0.0]]
		elif flagmode==2:
			return [['rec_ind','mean','median','re',1.8,0.3,1,'',0,0,0.0,0.0],['rec_ind','mean','median','im',1.8,0.3,1,'',0,0,0.0,0.0],\
					['vis_ind','mean','median','re',1.8,0.3,1,'',0,0,0.0,0.0],['vis_ind','mean','median','im',1.8,0.3,1,'',0,0,0.0,0.0],\
					['rec_ind','mean','median','re',1.7,0.3,1,'',0,0,0.0,0.0],['rec_ind','mean','median','im',1.7,0.3,1,'',0,0,0.0,0.0],\
					['vis_ind','mean','median','re',1.7,0.3,1,'',0,0,0.0,0.0],['vis_ind','mean','median','im',1.7,0.3,1,'',0,0,0.0,0.0]]
		else:
			return [['chan_ind','mean','median','re',1.8,0.3,1,'',0,0,0.0,0.0],['chan_ind','mean_rms','median','re',1.8,0.3,1,'',0,0,0.0,0.0],\
					...
					['chan_ind','mean','median','re',1.7,0.3,1,'',0,0,0.0,0.0],['chan_ind','mean_rms','median','re',1.7,0.3,1,'',0,0,0.0,0.0],\
					['chan_ind','mean','median','im',1.7,0.3,1,'',0,0,0.0,0.0],['chan_ind','mean_rms','median','im',1.7,0.3,1,'',0,0,0.0,0.0],\
					['rec_ind','mean','median','re',1.7,0.3,1,'',0,0,0.0,0.0],['rec_ind','mean_rms','median','re',1.7,0.3,1,'',0,0,0.0,0.0],\
					['rec_ind','mean','median','im',1.7,0.3,1,'',0,0,0.0,0.0],['rec_ind','mean_rms','median','im',1.7,0.3,1,'',0,0,0.0,0.0],\
					['vis_ind','mean','median','re',1.7,0.3,1,'',0,0,0.0,0.0],['vis_ind','mean_rms','median','re',1.7,0.3,1,'',0,0,0.0,0.0],\
					['vis_ind','mean','median','im',1.7,0.3,1,'',0,0,0.0,0.0],['vis_ind','mean_rms','median','im',1.7,0.3,1,'',0,0,0.0,0.0]]
	...
	def runankflag(self,inpfilename,ANTS,THREADS,nchan,nbaseline,ntime,npols,inp_fileformat='ms',out_fileformat='ms',overwrite=False,automode=True,datacolumn='corrected',\
		...
		if (WRITEOUT):
			if overwrite==True and inp_fileformat=='uvfits' and out_fileformat=='uvfits':
				...
				uvrange=float(msflag.range(['uvdist'])['uvdist'][-1])
				msflag.select({'uvdist':[0.00000000000000000000000000001,uvrange]})
					...
					if ntime!=1:
						for i in range(ntime):
							for j in range(nchan):
								flag_frac=float(len(np.where(flag[:,j,:,i].flatten()==True)[0]))/float(len(flag[:,j,:,i].flatten()))
						...
						for j in range(nchan):
							flag_frac=float(len(np.where(flag[:,j,:].flatten()==True)[0]))/float(len(flag[:,j,:].flatten()))
							if flag_frac>=chantime_flagfrac:
								if verbose:
				...
				if extendpols==True:
						...
						for p in range(npols):
							pos.append(np.where(flag[p,...]==True))
						for i in pos:
							for p in range(npols):
								flag[p,...][i]=True
				flag_data['flag']=flag
						...
						if ntime!=1:
							for i in range(ntime):
								for j in range(nchan):
									...
									if flag_frac>=chantime_flagfrac:
										if verbose:
											print ('Extending flag for chan : '+str(j)+' and time index : '+str(i)+'. Flag fraction : '+str(flag_frac)+'\n')
										flag[:,j,:,i]=flag[:,j,:,i]+True
						else:
							for j in range(nchan):
								flag_frac=float(len(np.where(flag[:,j,:].flatten()==True)[0]))/float(len(flag[:,j,:].flatten()))
								if flag_frac>=chantime_flagfrac:
									if verbose:
										print ('Extending flag for chan : '+str(j)+' and time index : '+str(0)+'. Flag fraction : '+str(flag_frac)+'\n')
									flag[:,j,:]=flag[:,j,:]+True
					if extendpols==True:
						if verbose:
							print ('Extending pol flags...\n')
