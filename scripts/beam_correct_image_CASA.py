...
def estimateSkyBrightnessMatrix_SLOW(j1, j2, Vij):
    my_shape = Vij.shape
    b = Vij * 0
    d=np.matrix([[0.0,-1.0],[-1.0,0.0]])
   # d=np.matrix([[0.0,1.0],[-1.0,0.0]])
    dinv=inv2x2(d)
    dH=np.transpose(d.conj(d))
    dHinv=inv2x2(dH)
    if len(my_shape) == 4:
        for i in range(my_shape[2]):
            for j in range(my_shape[3]):
                j1_2D = j1[:, :, i, j]
                j2_2D = j2[:, :, i, j]
		j1_2D=np.dot(d,j1_2D)
		j2_2D=np.dot(d,j2_2D)
                Vij_2D = Vij[:, :, i, j]
	...
	#	b_2D=np.dot(dinv,np.dot(b_2D,dHinv))
                b[:, :, i, j] = b_2D
...
def parse_options():
    ...
    parser.add_option('--xx_file',
                      ...
                      default=None,
                      help="Force xx_file")
    parser.add_option('--IQUV_file',
                      dest='iquv_file',
                      default=None,
                      help="Force IQUV_file")
...
if __name__ == "__main__":
   ...
   # Vij = np.array([[data_xx, data_xy + 1j * data_xyi], [data_xy - 1j * data_xyi, data_yy]])
    ...
    stokes = B2IQUV(B)#,sign_uv=sign_uv, sign_q=sign_q)
