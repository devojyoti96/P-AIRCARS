    ...
    if len(my_shape) == 4:
        for i in range(my_shape[2]):
            for j in range(my_shape[3]):
                ...
                j2_2D = j2[:, :, i, j]
                Vij_2D = Vij[:, :, i, j]
		j1_2D=np.dot(a,j1_2D)
		j2_2D=np.dot(a,j2_2D)
                j1inv = inv2x2(j1_2D)
...
if __name__ == "__main__":
    ...
    logger.info("########################################")
    logger.info("Beam correction code in IAU/IEEE convention considering MS is corrected into IAU convention"))
    ...
    logger.info('Get Stokes... %s ',% (datetime.datetime.now().time()))
