...
def split_func(params):
	...
	tb.flush()
	tb.close()
	#flag_MWA_coarse(outputvis,edgewidth=160,do_flag=True,force=True)
	applycal(vis=outputvis,gaintable=['/data1/devojyoti/PhD/data/paircars_test/basedir_for_2014_05_04/localdatabase/1083206896/1083206896_62_63.gcal'],\
...
def split_ms_parallel(paramList):
	...
	Parameters:
	paramList = Lists of list of parameters fosr each split, format [[vis1,datacolumn1,spw1,timerange1,chanwidth1,timewidth1],\
	[vis2,datacolumn2,spw2,timerange2,chanwidth2,timewidth2],....]
	Return:
	List of splited ms
	'''
	pool=mp.Pool(processes=int(mp.cpu_count()*0.6))	
...
if __name__=='__main__':
	...
	if options.savedir==None:
		savedir=os.path.dirname(os.path.abspath(str(options.msname)))
	elif os.path.isdir(str(options.savedir))==False:
		os.makedirs(str(options.savedir))
