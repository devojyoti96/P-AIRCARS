...
######################################################################
def get_azza_from_fits(filename, ext=0):
    """
...
def parse_options():
    ...
    parser.add_option('-a', '--az',
                      dest="az",
                      default="45",
                      help="Azimuth [default %default deg]",
                      metavar="FLOAT",
                      type="float")
    parser.add_option('-z', '--za',
                      dest="za",
                      default="19.6",
                      help="Zenith distance [default %default deg]",
                      metavar="FLOAT",
                      type="float")
    parser.add_option('-f', '--freq',
                      dest="freq",
                      default="205",
                      help="Freq in MHz [default %default MHz]",
                      metavar="FLOAT",
                      type="float")
    parser.add_option('-o', '--outfile',
                      dest="outfile",
                      default="test_jones",
                      help="FullFEE jones matrix output file")
    (options, args) = parser.parse_args()

    return (options, args)


def calc_jones(az, za, target_freq_Hz=205e6,outfile='test_jones'):
    za_rad = za * math.pi / 180.0
    az_rad = az * math.pi / 180.0
    za_rad = np.array([[za_rad]])
    az_rad = np.array([[az_rad]])
    ...
    delays = gridpoint[4]
    delays = np.vstack((delays, delays))
    print (za_rad,az_rad,target_freq_Hz,delays) 
    ...
    # TEST if equivalent to :
    Jones_FullEE_2D = Jones_FullEE_swap[:, :, 0, 0]
    a=np.matrix([[0.0,-1.0],[-1.0,0.0]])
    Jones_FullEE_2D_arranged = np.array([ [Jones_FullEE_swap[0, 0][0][0], Jones_FullEE_swap[0, 1][0][0]] , [Jones_FullEE_swap[1, 0][0][0], Jones_FullEE_swap[1, 1][0][0]] ])
    Jones_FEE_2D_IAU=np.matmul(a,Jones_FullEE_2D_arranged)
    print (Jones_FEE_2D_IAU)
    np.save(outfile,Jones_FEE_2D_IAU)
    '''
    print("Jones FullEE:")
    print("----------------------")
    print(Jones_FullEE)
...
if __name__ == "__main__":
   ...
   # outf.write("# DEC[deg]  S_xx/S_yy   Q_leakage GLEAM-RATIO-MS-way GLEAM-Ratio-DanielWAY\n")
    ...
    calc_jones(az, za, target_freq_Hz=target_freq_Hz,outfile=outfile)
