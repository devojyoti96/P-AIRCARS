...
def check_internet(url=''):
	...
	----------
	url : str
		An URL to check internet connectivity. (default : https://www.google.com)
	Returns
	-------
	bool
...
def get_OBSID_for_timerange(timerange='',caldata=True,project_ID='G0002'):
	'''
	Function to get MWA observation IDs for a given time range
	Parameters
	----------
	timerange : str
		Time range to search, format 'yy0/mm0/dd0/hh0:mm0:ss0.ff0~yy1/mm1/dd1/hh1:mm1:ss1.ff1,yy2/mm2/dd2/hh2:mm2:ss2.ff2~....'
	caldata : bool
		Whether search for calibrator observations also or not
	project_ID : str
		Project ID to search data
	Returns
	-------
	list
		OBSID list
	list
		Calibration OBSID list
	'''	
	BASEURL='http://ws.mwatelescope.org/'
	obsid_list=[]
	cal_obsid_list=[]
	if check_internet(url=BASEURL)==False:
		print ('No internet connection available. Could not search MWA database.\n')
		return [],[]
	else:
		timelist=timerange.split(',')
		for i in timelist:
			starttime=mjdsec_to_timestamp(timestamp_to_mjdsec(i.split('~')[0],format=0),includedate=True,format=1)		
			endtime=mjdsec_to_timestamp(timestamp_to_mjdsec(i.split('~')[-1],format=0),includedate=True,format=1)
			cal_starttime=mjdsec_to_timestamp(int(timestamp_to_mjdsec(i.split('~')[0],format=0))-1,includedate=True,format=1)		
			cal_endtime=mjdsec_to_timestamp(int(timestamp_to_mjdsec(i.split('~')[-1],format=0))+1,includedate=True,format=1)					
			searchurl=BASEURL+'metadata/tconv/?utciso='+starttime
			startGPStime=json.load(urllib.request.urlopen(searchurl,timeout=10))
			searchurl=BASEURL+'metadata/tconv/?utciso='+endtime
			endGPStime=json.load(urllib.request.urlopen(searchurl,timeout=10))
			searchurl=BASEURL+'metadata/find?maxtime='+str(endGPStime)+'&mintime='+str(startGPStime)+'&dataquality=1&notdeleted=on&projectid='+str(project_ID)+'&page=20000000000000'
			try:
				OBSids=json.load(urllib.request.urlopen(searchurl,timeout=15))
				for i in range(len(OBSids)):
					obsid_list.append(int(OBSids[i][0]))
			except:
				pass
			searchurl=BASEURL+'metadata/tconv/?utciso='+cal_starttime
			startGPStime=json.load(urllib.request.urlopen(searchurl,timeout=10))
			searchurl=BASEURL+'metadata/tconv/?utciso='+cal_endtime
			endGPStime=json.load(urllib.request.urlopen(searchurl,timeout=10))
			searchurl=BASEURL+'metadata/find?maxtime='+str(endGPStime)+'&mintime='+str(startGPStime)+'&dataquality=1&calibration=on&notdeleted=on&page=20000000000000'
			try:
				OBSids=json.load(urllib.request.urlopen(searchurl,timeout=15))
				for i in range(len(OBSids)):
					if int(OBSids[i][0]) not in cal_obsid_list:
						cal_obsid_list.append(int(OBSids[i][0]))
			except:
				pass
		return obsid_list,cal_obsid_list
