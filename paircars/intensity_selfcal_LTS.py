...
class IntensitySelfcal:
	...
	maximum_emission_scale : float 
		Maximum scale of the emission present in the image in arcsec
	largest_scale : float 
	...
	def calc_dyn_range(self,num_iter,sigma,box_width=3,stokes_list=['I']):
		...
		-------
		dict	
			Dynamic range information {'STOKES':[rms dynamic range,rms,total_flux(non-negative)]}
		float
	...
	def change_start_sigma(self,nsigma,sigma_step,min_sigma):
		...
		nsigma : float 
			Present sigma value
		sigma_step : float 
			Sigma reduction step
		min_sigma : float 
			Minimum value of sigma to reduce
		Returns
		-------
		float
	...
	def calc_iter_num(self,safety_factor,quality_factor,scratch=True,bandpass_selfcal=False):
		...
		scratch : bool
			Whether the selfcal started from scratch or not
		bandpass_selfcal : bool
			Performing bandpass selfcal or not
		Returns
		-------
		int
	...
	def DR_record(self,DR,record_filename,init=True):
		...
		----------
		DR : float 
			Dynamic range of the current selfcal iteration to record
		record_filename : str 
			Name of the file to store dynamic ranges
		init : bool
	...
	def DR_delta(self,quality_factor,DR_array,frac_increase,safety_factor): # Not tested and not used
		...
		----------
		quality_factor : int 
			Factor to determine the image quality
		DR_array : numpy.array 
			Dynamic range records array
		frac_increase : float 
			Fractional increase of the DR step from previous DR steps
		safety_factor : int 
			Factor to determine the robustness of the selfcal
		Returns
		-------
	...
	def calc_selfcal_snr(self,imagename,sigma,num_antenna_to_use):
		...
		imagename : str 
			Name of the image
		sigma : float 
			Present sigma value used for making the image
		num_antenna_to_use : int 
			Number of antenna used for imaging
		Returns
		-------
		float
	...
	def dirty_image(self,start_sigma,box_width=3,antenna_to_use='',weight='briggs',robust=0.5):
		'''
		...
		float
			Per antenna SNR for self-calibration
		int
			Error message code
		'''
	...
	def file_remover_and_keeper(self,num_iter,msg_code,do_bandpass=False,ref_time_chan=True):
		...
		if num_iter=='dirty':
			...
			os.system('rm -rf '+file_str+'.image '+file_str+'.model '+file_str+'.residual '+file_str+'.sumwt '+file_str+'.pb '+file_str+'.psf *psf*') 
			os.chdir(self.cwd)
			os.system('rm -rf casa*log')
			return
		if num_iter==0 and (msg_code==0 or msg_code==8 or msg_code==9):
			...
			elif os.path.isdir(file_str+'.mask'):
				os.system('cp -r '+file_str+'.mask junk0.mask') # Copying model to junk0.mask
			...
			os.system('rm -rf junk1.ms junk1.cal junk1.model junk1.mask junk1.image junk1.residual *psf*')
	...
	def initial_bpass(self,modelname,num_iter,rms_thresh,ref_ant,minsnr,calmode,calibrator_caltable=[]):
		...
		----------
		modelname : str 
			Name of the initial model
		num_iter : int
			Selfcal iteration number
		rms_thresh : list
			List of n-sigma value for rms based flagging (Do not go below 6)
		ref_ant : int 
			Reference antenna number
		calmode : str
			'p' for phase-only or 'ap' for amplitude-phase calibration
		calibrator_caltable : list 
			List of any previous caltables
		Returns
		-------
		str
		...
		self.log_verbose.info('applycal(vis=\''+self.msname+'\',gaintable='+str(calibrator_caltable)+',applymode=\'calflag\',calwt=[False],flagbackup=True)\n')
		applycal(vis=self.msname,gaintable=calibrator_caltable,applymode='calflag',calwt=[False],flagbackup=True)
		os.system('rm -rf casa*log')
	...
	def bpass_solver(self,caltable,spw='',timerange='',calmode='ap',uvrange='',solnorm=True,refant='1',minsnr=3,solmode='',rmsthresh=[],gaintable=[]):
		...
		solnorm : bool
			Normalise solution or not
		refant : str 
			Reference antenna
		minsnr : float 
			Minimum gain SNR
		solmode : str 
			'R' for robust calibration
		rmsthresh : list 
			List of n-sigma values for rms based flagging (Do not put below 6)
		gaintable : list
			List of any previous caltables
		Returns 
		-------- 
		...
		if solmode!='R' or len(rmsthresh)==0:
			...
			for rms in rmsthresh:
				...
				while c==0:
					...
					cb.solve()
					cb.close()
					applycal(vis=visname,gaintable=gaintable+[caltable],applymode='calflag',calwt=[False],flagbackup=False)
	...
	def cal_solar_phaseshift(self,imagename,sigma=10):
		'''
		...
		imagename : str 
			Name of the image
		sigma : float 
			Threshold for model fitting (default =10)
		Returns
		-------
		float
			New RA of the phasecenter in degree
		float
			New DEC of the phasecenter in degree
		bool
	...
	def image_source_true_loc(self,outdir,do_bandpass=False,weight='briggs',robust=0.5):
		'''
		...
		Outdir : str 
			Output directory
		do_bandpass : bool 
			Source true location for bandpass self-calibration or not
		weight : str
			Visibility weighting during imaging , 'uniform', 'natural' or 'briggs'. Default is 'briggs'
		robust : float
			Robust parameter for briggs weighting, default : 0.5 
	...
	def selfcal_iteration(self,num_iter,rms_thresh,sigma,maskstr,antenna_to_use,startmodel,startmask,ref_ant,minsnr,calmode,maskfile='',want_auto_masking=False,cpus=5,absmem=10,\
		...
		num_iter : int 
			Number of self-calibration iteration
		rms_thresh : list 
			List of rms for threshold
		sigma : float 
			Threshold sigma
		maskstr : str 
			CASA mask string for CLEANing (For CASA only)
		antenna_to_use : list 
		...
		ref_ant : int
			Reference antenna
		minsnr : float 
			Minimum SNR of gain solutions
		calmode: str 
			'p' for phase-only and 'ap' for amplitude-phase calibration
		maskfile : str 
			Name of the maskfile
		...
		interactive : bool
			Perform interactive selfcal, change options on the fly
		do_bandpass : str
			Perform bandpass calibration
		solmode : str
			'R', 'L1R', 'L1' for gaincal and only 'R' for bandpass
		correct_phasecenter : bool 
			Correct the phase center
		ra : float 
			New RA in degree to change phasecenter
		dec : float 
			New DEC in degree to change phasecenter
		box_width : float 
			Negative box width around the maximum pixel in degree (default : 3 degree)
		calibrator_caltable : list 
		...
		if os.path.isdir(casa_modelname)==False:
			...
			if modelflux==0.0:
				...
				self.log_verbose.info('ft(vis=\''+self.msname+'\',model=\''+casa_modelname+'\',nterms=1,usescratch=True)\n') 
