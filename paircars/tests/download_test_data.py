...
def download_with_parfive(record_id, update=False, output_dir="zenodo_download"):
    print("####################################")
    print("Downloading P-AIRCARS test data files ...")
    print("####################################")
...
def check_test_data(path):
    if os.path.exists(path + "/testdata/.testdata") == False:
        ...
        download_with_parfive(18625377, output_dir=path + "/testdata/")
        shutil.unpack_archive(
            path + "/testdata/paircars_test_data.tar.gz",
            extract_dir=f"{path}/testdata",
        )
        os.system("rm -rf " + path + "/testdata/paircars_test_data.tar.gz")
