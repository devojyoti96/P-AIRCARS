...
@patch(
    "paircars.pipeline.do_imaging.rename_mwasolar_image",
    side_effect=lambda *args, **kwargs: f"/mock/renamed/{os.path.basename(args[0])}",
)
@patch(
    "paircars.pipeline.do_imaging.make_stokes_wsclean_imagecube",
    side_effect=lambda images, output, **kwargs: output,
)
@patch(
    "paircars.pipeline.do_imaging.glob.glob",
    side_effect=lambda pattern: [pattern.replace("*", "I")],
)
@patch("paircars.pipeline.do_imaging.run_wsclean", return_value=0)
@patch("paircars.pipeline.do_imaging.get_multiscale_bias", return_value=0.7)
@patch("paircars.pipeline.do_imaging.calc_multiscale_scales", return_value=[0, 4, 16])
@patch("paircars.pipeline.do_imaging.calc_sun_dia", return_value=30)
...
@patch("paircars.pipeline.do_imaging.timestamp_to_mjdsec", side_effect=lambda t: 0)
@patch("paircars.pipeline.do_imaging.msmetadata")
@patch("paircars.pipeline.do_imaging.clean_shutdown")
@patch("paircars.pipeline.do_imaging.time.sleep", return_value=None)
@patch("paircars.pipeline.do_imaging.drop_cache")
...
@patch("paircars.pipeline.do_imaging.drop_cache")
@patch("paircars.pipeline.do_imaging.os.system")
@patch("paircars.pipeline.do_imaging.os.makedirs")
@patch("paircars.pipeline.do_imaging.calc_sun_dia", return_value=32.0)
...
@patch("paircars.pipeline.do_imaging.check_udocker_container")
@patch("paircars.pipeline.do_imaging.check_datacolumn_valid")
@patch("paircars.pipeline.do_imaging.init_logger")
@patch("paircars.pipeline.do_imaging.create_logger")
@patch("paircars.pipeline.do_imaging.msmetadata")
@patch("paircars.pipeline.do_imaging.get_local_dask_cluster")
@patch("paircars.pipeline.do_imaging.np.load", return_value=["job", "pass"])
@patch("paircars.pipeline.do_imaging.perform_imaging")
...
def test_cli_do_imaging(mock_print_help, mock_exit, mock_main, argv, should_exit):
    with patch("sys.argv", argv):
        from paircars.pipeline import do_imaging
