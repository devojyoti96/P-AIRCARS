...
@patch("paircars.pipeline.init_data.requests.get")
...
@patch("paircars.pipeline.init_data.os.makedirs")
@patch("paircars.pipeline.init_data.os.system")
@patch("paircars.pipeline.init_data.psutil.cpu_count", return_value=4)
@patch("paircars.pipeline.init_data.Downloader")
@patch("paircars.pipeline.init_data.get_zenodo_file_urls")
@patch("paircars.pipeline.init_data.os.path.exists")
@patch("paircars.pipeline.init_data.all_filenames", new=["mockfile.txt"])
...
@patch("paircars.pipeline.init_data.download_with_parfive")
@patch("paircars.pipeline.init_data.os.path.exists")
@patch("paircars.pipeline.init_data.open", new_callable=mock_open)
@patch("paircars.pipeline.init_data.os.getlogin", return_value="mockuser")
@patch("paircars.pipeline.init_data.get_cachedir", return_value="/mock/cache")
@patch("paircars.pipeline.init_data.get_datadir", return_value="/mock/data")
@patch("paircars.pipeline.init_data.os.makedirs")
def test_init_paircars_data(
    mock_makedirs,
    mock_get_datadir,
    mock_get_cachedir,
    mock_getlogin,
    ...
    import paircars.pipeline.init_data as init_data
    ...
    mock_exists.side_effect = side_effect
    # Run function
    init_paircars_data(update=update, remote_link=remote_link, emails=emails)
    # Download check
    if expect_download:
        mock_download.assert_called_once_with(
            "18640418", update=update, output_dir="/mock/data"
        )
    else:
...
@patch("paircars.pipeline.init_data.main", return_value=0)
@patch("paircars.pipeline.init_data.sys.exit")
@patch("paircars.pipeline.init_data.argparse.ArgumentParser.print_help")
