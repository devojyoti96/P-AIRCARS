...
def master_control(
    ...
    try:
            ...
            try:
                ...
                if adaptive and len(filtered_ms) != len(target_mslist):
                    scale_worker_and_wait(
                        dask_cluster,
                        dask_client,
                        min(len(target_mslist), max_worker),
                    )
        ...
        if calibrator_obsid is not None:
            ...
            bandpass_tables = glob.glob(f"{caldir}/calibrator_{calibrator_obsid}*.bcal")
            print(
            ...
            if do_selfcal:
                ...
                if adaptive:
                    scale_worker_and_wait(
                        dask_cluster,
                        dask_client,
                        min(len(selfcal_mslist), max_worker),
                    )
                if do_sidereal_cor:
                    if emails != "":
        ...
        if (do_applycal and has_cal) or do_apply_selfcal or do_imaging:
            if adaptive:
                scale_worker_and_wait(
                    dask_cluster,
                    dask_client,
                    min(len(split_target_mslist), max_worker),
                )
            ############################
            # Basic flagging
            ############################
