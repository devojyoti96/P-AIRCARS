...
def poltclean(vis="",selectdata=True,field="",spw="",timerange="",uvrange="",\
			antenna="",scan="",observation="",intent="",datacolumn="corrected",imagename="",imsize=[100],cell="1.0arcsec",phasecenter="",\
			stokes="I",projection="SIN",startmodel="",startmask="",specmode="mfs",reffreq="",nchan=-1,start="",width="",outframe="LSRK",veltype="radio",restfreq=[],\
			interpolation="linear",perchanweightdensity=False,gridder="standard",facets=1,psfphasecenter="",chanchunks=1,wprojplanes=1,vptable="",\
			usepointing=False,mosweight=True,aterm=True,psterm=False,wbawp=True,conjbeams=False,cfcache="",computepastep=360.0,rotatepastep=360.0,pblimit=0.2,\
	...
	Arguments :
		...
		cell:	Cell size
			   ...
			   if cell = '1' given without any unit, default unit arcsec will be taken 
		...
		startmask: Name of the start mask
				
				This mask will be copied at the initial state and go for further masking
		
				If startmask is used mask parameter will be ignored

				example startmask='try.mask'
		...
		perchanweightdensity:	
                         ...
                         density for each channel independently or a
                         common weight density for all of the selected
                         data (the default). This parameter has no
                         meaning for continuum (specmode='mfs')
                         imaging but for cube imaging
                         perchanweightdensity=True is a recommended
                         alternative option that provides more uniform
		...
		usepointing:	Use the pointing table to determine where the beam are for mosaic gridder; if False then phasecenters of the fields selected are used to determine direction of each mosaic pointing.
		...
		threshold:	Stopping threshold (number in units of Jy, or string),  List of threshold for individual stokes planes
                      ...
                      threshold = ['5.0mJy','2.0mJy'] if two stokes planes are present
		...
		casalogger: Print logger in terminal, Default value : False
	...
	if selectdata==True:
		pc.field=field
		pc.spw=spw
		pc.timestr=timerange
		pc.uvdist=uvrange
		pc.antenna=antenna
		pc.scan=scan
		pc.obs=observation
	pc.datacolumn=datacolumn
	pc.imagename=imagename
	pc.imsize=imsize
	if type(cell)==int or type(cell)==float:
		cell=str(cell)
	if 'arcsec' not in cell:
		if 'arcmin' not in cell:
			cell=cell+'arcsec'
	pc.cell=cell
	pc.phasecenter=phasecenter
	pc.stokes=stokes
	pc.projection=projection
	pc.startmodel=startmodel
	pc.specmode=specmode
	if specmode=='cube' or specmode=='cubedata' or specmode=='cubesource':
		pc.reffreq=reffreq
		pc.nchan=nchan
		pc.start=start
		pc.width=width
		pc.outframe=outframe
		pc.veltype=veltype
		pc.restfreq=restfreq
		pc.interpolation=interpolation
		pc.chanchunks=chanchunks
		pc.perchanweightdensity=perchanweightdensity
	pc.gridder=gridder
	pc.facets=facets
	pc.wprojplanes=wprojplanes
	pc.pblimit=pblimit
	pc.deconvolver=deconvolver
	pc.scales=scales
	pc.nterms=nterms
	pc.scalebias=smallscalebias
	pc.loopgain=gain
	...
	pc.uvtaper=uvtaper
	pc.niter=niter
	pc.cycleniter=cycleniter
	pc.usemask=usemask
	pc.savemodel=savemodel
	pc.vptable=vptable
	pc.usepointing=usepointing
	pc.mosweight=mosweight
	pc.aterm=aterm
	pc.psterm=psterm      
	pc.wbawp=wbawp            
	pc.conjbeams=conjbeams    
	pc.cfcache=cfcache           
	pc.computepastep=computepastep
	pc.rotatepastep=rotatepastep
	pc.normtype=normtype  
	pc.restoringbeam=restoringbeam
	pc.dopbcor=pbcor
	pc.outlierfile=outlierfile 
	pc.nsigma=nsigma    
	pc.cyclefactor=cyclefactor
	pc.minpsffraction=minpsffraction     
	pc.maxpsffraction=maxpsffraction 
	pc.pbmask=pbmask         
	pc.sidelobethreshold=sidelobethreshold 
	pc.noisethreshold=noisethreshold 
	pc.lownoisethreshold=lownoisethreshold
	pc.negativethreshold=negativethreshold
	pc.smoothfactor=smoothfactor
	pc.minbeamfrac=minbeamfrac       
	pc.cutthreshold=cutthreshold    
	pc.growiterations=growiterations
	pc.dogrowprune=dogrowprune
	pc.minpercentchange=minpercentchange
	pc.verbose=verbose
	pc.fastnoise=fastnoise        
	pc.restart=restart           
	pc.calcres=calcres        
	pc.calcpsf=calcpsf        
