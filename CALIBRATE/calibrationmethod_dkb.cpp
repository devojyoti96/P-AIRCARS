...
double my_maxfinder(double *a,int n)//, int *maxpos)
{
	double b = 0.0;
	int i;
	for (i=0;i<n;i++)
...
double frobeniusNorm(std::complex<double> mat[4])
{
    double sumSq = 0.0;
    for (int i = 0; i < 4; i++) {
            sumSq += std::norm(mat[i]);
    }
    double res = sqrt(sumSq);
    return res;
}

double CalibrationMethod::calculateChisq(std::complex<double>* nextJones)
{
	double chival=0.0;	
	for(size_t timestep=0; timestep!=_nTimesteps; ++timestep)
	{
		for(size_t antenna1 = 0; antenna1!=_nAntenna; ++antenna1)
		{
			...
			if(antenna1 != antenna)
			{
				...
				const std::complex<double> *jones1 = &nextJones[antenna*4];
				const std::complex<double> *jones2 = &nextJones[antenna1*4];
				
				std::complex<double> j1TimesM[4], j1MJ2H[4], distances[4];
				for(size_t ch=0; ch!=_nChannels; ++ch)
				{
					if(isConjTranspose)
					{
						j1TimesM[0] = jones1[0] * std::conj(model[0]) + jones1[1] * std::conj(model[2]);
						j1TimesM[1] = jones1[0] * std::conj(model[1]) + jones1[1] * std::conj(model[3]);
						j1TimesM[2] = jones1[2] * std::conj(model[0]) + jones1[3] * std::conj(model[2]);
						j1TimesM[3] = jones1[2] * std::conj(model[1]) + jones1[3] * std::conj(model[3]);
						
						j1MJ2H[0] = j1TimesM[0] * std::conj(jones2[0]) + j1TimesM[1] * std::conj(jones2[2]);
						j1MJ2H[1] = j1TimesM[0] * std::conj(jones2[1]) + j1TimesM[1] * std::conj(jones2[3]);
						j1MJ2H[2] = j1TimesM[2] * std::conj(jones2[0]) + j1TimesM[3] * std::conj(jones2[2]);
						j1MJ2H[3] = j1TimesM[2] * std::conj(jones2[1]) + j1TimesM[3] * std::conj(jones2[3]);
						
						distances[0] = std::conj(data[0]) - j1MJ2H[0];
						distances[1] = std::conj(data[2]) - j1MJ2H[1],
						distances[2] = std::conj(data[1]) - j1MJ2H[2];
						distances[3] = std::conj(data[3]) - j1MJ2H[3];
					} else {
						j1TimesM[0] = jones1[0] * model[0] + jones1[1] * model[2];
						j1TimesM[1] = jones1[0] * model[1] + jones1[1] * model[3];
						j1TimesM[2] = jones1[2] * model[0] + jones1[3] * model[2];
						j1TimesM[3] = jones1[2] * model[1] + jones1[3] * model[3];
						
						j1MJ2H[0] = j1TimesM[0] * std::conj(jones2[0]) + j1TimesM[1] * std::conj(jones2[1]);
						j1MJ2H[1] = j1TimesM[0] * std::conj(jones2[2]) + j1TimesM[1] * std::conj(jones2[3]);
						j1MJ2H[2] = j1TimesM[2] * std::conj(jones2[0]) + j1TimesM[3] * std::conj(jones2[1]);
						j1MJ2H[3] = j1TimesM[2] * std::conj(jones2[2]) + j1TimesM[3] * std::conj(jones2[3]);
						
						distances[0] = data[0] - j1MJ2H[0];
						distances[1] = data[1] - j1MJ2H[1],
						distances[2] = data[2] - j1MJ2H[2];
						distances[3] = data[3] - j1MJ2H[3];
					}
					
					chival += (frobeniusNorm(distances)*frobeniusNorm(distances));
...
double det(std::complex<double> mat[4])
{
...
void CalibrationMethod::Execute(double& precisionLimit, size_t& nIter)
{
	...
	{
		double s1,s2;
		SolutionSingularValue(ant, s1, s2);
